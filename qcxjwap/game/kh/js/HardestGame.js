...
// 	alert("试试")
// })
var gameApiSuccess = false;
var lipstickGame = null;
(function() {
	ajaxPost(baseUrl + 'lipstickGame/get', {
		id: getQueryString('id') != null ? getQueryString('id') : 1
	}, function(res) {
		lipstickGame = res.lipstickGame;
		console.log(lipstickGame);
		$('.layoutRoot').css({
			'background-image': 'url("' + fileUrl + lipstickGame.backFilePath + '")'
		});
		$('body').css({
			'background': 'url("' + fileUrl + lipstickGame.backFilePath + '")repeat-x'
		});
		$('html').css({
			'background': 'url("' + fileUrl + lipstickGame.backFilePath + '")repeat-x'
		});

		$('#levelSwitchBoxMain').attr({
			'src': fileUrl + lipstickGame.level1FilePath
		})
		...
		var CircleCenterSplit = [{
				"left": {
					"url": "img/CircleCenter_1_split_left.png",
					"w": "238",
					"h": "407",
					"rs": Math.PI * 2
				},
				"right": {
					"url": "img/CircleCenter_1_split_right.png",
					"w": "230",
					"h": "407",
					"cutx": "26",
					"rs": Math.PI * 2
			...
			{
				"left": {
					"url": "img/CircleCenter_2_split_left.png",
					"w": "238",
					"h": "407",
					"rs": Math.PI * 2
				},
				"right": {
					"url": "img/CircleCenter_2_split_right.png",
					"w": "230",
					"h": "407",
					"cutx": "26",
					"rs": Math.PI * 2
			...
			{
				"left": {
					"url": "img/CircleCenter_3_split_left.png",
					"w": "238",
					"h": "407",
					"rs": Math.PI * 2
				},
				"right": {
					"url": "img/CircleCenter_3_split_right.png",
					"w": "230",
					"h": "407",
					"cutx": "26",
					"rs": Math.PI * 2
		...
		//关卡资源
		var levelUrl = [{
				"normal": "img/level_icon_1_active.png",
				"active": "img/level_icon_1_active.png"
			},
			{
				"normal": "img/level_icon_2.png",
				"active": "img/level_icon_2_active.png"
			},
			{
				"normal": "img/level_icon_3.png",
				"active": "img/level_icon_3_active.png"
			}
		]
		...
		var D_Json = {
			"Level3_PARAMETERS": {
				...
				"rotationAccelerationSpeed": 0.007,
				"levelArray": [0, lipstickGame.level3LipstickCount, 0.07, lipstickGame.level3GameTime]
			},
			"Level2_PARAMETERS": {
				"ROTAION_SPEED_ARRAY": [-0.06, -0.03, -0.02, 0.02, 0.04, 0.06],
				"rotationAccelerationSpeed": 0.002,
				"levelArray": [0, lipstickGame.level2LipstickCount, 0.04, lipstickGame.level2GameTime]
			},
			"Level1_PARAMETERS": {
				"ROTAION_SPEED_ARRAY": [-0.05, -0.04, -0.02, 0.02, 0.04, 0.05],
				"rotationAccelerationSpeed": 0.001,
				"levelArray": [0, lipstickGame.level1LipstickCount, 0.02, lipstickGame.level1GameTime]
				//    [0,口红数，速度，时间]
			},
			"LAST_3_PARAMETERS": {
				"ROTAION_SPEED_ARRAY": [-0.09, -0.07, -0.05, 0.06, 0.08, 0.09],
		...
		CircleCenter.prototype.resetCircleCenterSplit = function(context, that, centerCircleSplitReposition, rs) {
			...
			var translatePosition = {
				left: {
					x: ROTATION_CENTER.x + centerCircleSplitReposition.left.x - left_w * 0.5,
					y: this.y + centerCircleSplitReposition.left.y
				},
				right: {
					x: ROTATION_CENTER.x - right_cutx + centerCircleSplitReposition.right.x + right_w * 0.5,
					y: this.y + centerCircleSplitReposition.right.y
				}
			}
			var radius = this.radius;
			/*左边的碎片*/
		...
		Game.prototype = {
			...
			normalResults: function() {
				...
				if(BulletCirclesIndex == lens) {
					...
					this.AimBullet = new CircleBullet(ROTATION_CENTER.x, AimBulletBottom, AimBullet.w, AimBullet.h, AimBulletImg,
						Math.PI * 0.5, false, null, null);
				}
			},
			...
			nextLevel: function() {
				...
				if(self.level <= self.levelLength) {
					...
					switch(self.level) {
						case 1:
							filePath = lipstickGame.level1FilePath;
							break;
						case 2:
							filePath = lipstickGame.level2FilePath;
							break;
						case 3:
							filePath = lipstickGame.level3FilePath;
							break;
					}
					$("#levelSwitchBoxMain").attr("src", fileUrl + filePath);
			...
			checkCollision: function(circleBullet) {
				...
				for(var i = 0; i < this.rotationCircles.length; i++) {
					...
					var A_l_t = {
						x: Ax - Aw / 2 * Math.sin(θ),
						y: Ay + Aw / 2 * Math.cos(θ)
					}
					var A_r_t = {
						x: Ax + Aw / 2 * Math.sin(θ),
						y: Ay - Aw / 2 * Math.cos(θ)
					...
					var A_r_b = {
						x: Ax + (Ah - Az) * Math.cos(θ) + Aw / 2 * Math.sin(θ),
						y: Ay + (Ah - Az) * Math.sin(θ) - Aw / 2 * Math.cos(θ)
					}
					var A_l_b = {
						x: Ax + (Ah - Az) * Math.cos(θ) - Aw / 2 * Math.sin(θ),
						y: Ay + (Ah - Az) * Math.sin(θ) + Aw / 2 * Math.cos(θ)
